  ...
  return (
    ...
    >
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="grid grid-cols-3 gap-1.5 mb-2">
              ...
              <div className="bg-slate-50 dark:bg-slate-800/80 rounded-lg p-2 text-center">
                <p className="text-[9px] t-muted uppercase tracking-wide mb-0.5">Vacance</p>
                <p className={`text-xs font-bold ${n(city.insee?.vacancyRate) > 10 ? 'text-red-600 dark:text-red-400' : 't-primary'}`}>
            ...
            {/* Short verdict */}
            {ins?.shortVerdict && (
              <p className="text-[10px] t-muted leading-relaxed mb-2 italic">{ins.shortVerdict}</p>
            )}
      ...
      <div className="border-t border-slate-100 dark:border-slate-800 px-4 py-2 flex items-center justify-between">
        <span className="text-[10px] t-muted">Transport : {fmt.score(city.transport?.transportScore)} · Revenu : {city.insee?.medianIncome ? fmt.num(city.insee.medianIncome) + '€' : '—'}</span>
