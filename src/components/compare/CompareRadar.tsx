...
interface Props { cities: City[]; onRemove: (inseeCode: string) => void; }
          ...
          <table className="w-full min-w-[400px]">
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50">
                ...
                {cities.map((city, i) => (
                  <th key={city.geo?.inseeCode ?? city.city} className="px-4 py-3 text-left">
            ...
            <tbody>
              {ROWS.map(({ label, getValue }, i) => (
                <tr key={label} className={`border-b border-slate-100 dark:border-slate-900 ${i % 2 === 0 ? '' : 'bg-slate-50/50 dark:bg-slate-900/20'}`}>
                  ...
                  {cities.map((city) => (
                    <td key={city.geo?.inseeCode ?? city.city} className="px-4 py-2.5">{getValue(city)}</td>
                  ))}
                </tr>
              ))}
            </tbody>
