...
const PROFILE_STYLES: Record<InvestmentProfile, { label: string; cls: string }> = {
  YIELD_TRAP:            { label: 'Piège rendement',       cls: 'bg-red-100 dark:bg-red-950 text-red-700 dark:text-red-400 border-red-200 dark:border-red-900' },
  ...
  BEGINNER_FRIENDLY:     { label: 'Débutant',              cls: 'bg-blue-100 dark:bg-blue-950 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-900' },
  PATRIMONIAL_SAFE:      { label: 'Patrimonial',           cls: 'bg-violet-100 dark:bg-violet-950 text-violet-700 dark:text-violet-400 border-violet-200 dark:border-violet-900' },
  ...
  BALANCED_OPPORTUNITY:  { label: 'Équilibré',             cls: 'bg-teal-100 dark:bg-teal-950 text-teal-700 dark:text-teal-400 border-teal-200 dark:border-teal-900' },
  LOW_INTEREST:          { label: 'Faible intérêt',        cls: 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700' },
  DATA_INCOMPLETE:       { label: 'Données incomplètes',   cls: 'bg-amber-100 dark:bg-amber-950 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-900' },
};
...
const RISK_STYLES: Record<RiskLevel, { label: string; cls: string; dot: string }> = {
  ...
  HIGH:      { label: 'Risque élevé',       cls: 'bg-orange-100 dark:bg-orange-950 text-orange-700 dark:text-orange-400 border-orange-200 dark:border-orange-900', dot: 'bg-orange-500' },
  VERY_HIGH: { label: 'Risque très élevé',  cls: 'bg-red-100 dark:bg-red-950 text-red-700 dark:text-red-400 border-red-200 dark:border-red-900', dot: 'bg-red-500' },
};

const RECO_STYLES: Record<Recommendation, { label: string; cls: string }> = {
  STRONG_OPPORTUNITY:  { label: 'Forte opportunité',       cls: 'bg-emerald-100 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-900' },
  GOOD_TO_ANALYZE:     { label: 'À analyser',              cls: 'bg-blue-100 dark:bg-blue-950 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-900' },
  ONLY_EXPERIENCED:    { label: 'Expérimenté uniquement',  cls: 'bg-amber-100 dark:bg-amber-950 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-900' },
  AVOID_FOR_BEGINNER:  { label: 'Éviter si débutant',      cls: 'bg-orange-100 dark:bg-orange-950 text-orange-700 dark:text-orange-400 border-orange-200 dark:border-orange-900' },
...
const TRANSPORT_STYLES: Record<TransportClassification, { label: string; cls: string }> = {
  ISOLATED:  { label: 'Isolée',              cls: 'bg-red-100 dark:bg-red-950 text-red-700 dark:text-red-400 border-red-200 dark:border-red-900' },
  LOW:       { label: 'Transport faible',    cls: 'bg-orange-100 dark:bg-orange-950 text-orange-700 dark:text-orange-400 border-orange-200 dark:border-orange-900' },
  MODERATE:  { label: 'Transport moyen',     cls: 'bg-amber-100 dark:bg-amber-950 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-900' },
...
export function ProfileBadge({ profile, small }: BadgeProps & { profile: InvestmentProfile }) {
  ...
  if (!s) return null;
  return <span className={`inline-flex items-center border rounded-full font-medium ${small ? 'text-[10px] px-1.5 py-0.5' : 'text-xs px-2 py-0.5'} ${s.cls}`}>{s.label}</span>;
}
