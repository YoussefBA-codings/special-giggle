...
import { Calculator, Home, Clock, Landmark, AlertTriangle } from 'lucide-react';
  ...
  return (
    <div className="card p-5">
      <div className="flex items-center gap-3 mb-5">
        ...
        <h2 className="text-sm font-bold t-primary">{title}</h2>
      </div>
      {children}
    </div>
  );
...
export function SimulateurRentabilitePage() {
  // Step 1
  ...
  const t3 = useMemo(
    () => calcT3Max(loyer, ptz, t1, t2, taux, duree),
    [loyer, ptz, t1, t2, taux, duree]
  );
  ...
  return (
    <div className="p-4 sm:p-6 page-bg min-h-full">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black t-primary">Simulateur de rentabilité locative</h1>
            <p className="text-sm t-muted">
              Calcule le budget max d'acquisition basé sur l'autofinancement total du bien
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
                ...
                <div className="p-4 rounded-xl bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="px-1.5 py-0.5 rounded-full bg-emerald-200 dark:bg-emerald-900 text-emerald-700 dark:text-emerald-300 text-[9px] font-bold uppercase tracking-wide">
                        calculé
                      </span>
                    </div>
                  ...
                  <p className="text-[11px] text-emerald-600/80 dark:text-emerald-400/80 mt-1.5">
                    Montant max empruntable pour que l'échéance totale = loyer mensuel
                  </p>
                </div>
          ...
          <div className="xl:col-span-1 space-y-4 xl:sticky xl:top-4 xl:self-start">
            ...
            <div className="space-y-2">
              {[
                { label: 'Loyer mensuel', value: eur(loyer), sub: null },
                {
                ...
                <div key={m.label} className="elevated rounded-xl p-3">
                  ...
                  <p
                    className={`text-lg font-black ${
                      m.highlight ? 'text-emerald-600 dark:text-emerald-400' : 't-primary'
                    }`}
                  >
                    {m.value}
                  </p>
                  {m.sub && (
                    <p
                      className={`text-[10px] mt-0.5 ${
                        m.highlight ? 'text-emerald-500' : 't-muted'
                      }`}
                    >
                      {m.sub}
                    </p>
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-800">
                {tranches.map((tr) => (
                  ...
                  >
                    <div className="flex items-center gap-1.5 mb-0.5">
                      ...
                      {tr.calculated && (
                        <span className="px-1 py-px rounded bg-emerald-100 dark:bg-emerald-900 text-emerald-700 dark:text-emerald-400 text-[9px] font-bold">
                          calculé
                        </span>
                      )}
                    </div>
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="font-semibold t-primary tabular-nums">
                        {eur(tr.mens)} /mois
                      </span>
                    </div>
                  </div>
                ...
                <div className="px-4 py-3 bg-slate-100 dark:bg-slate-800">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-black t-primary tabular-nums">
                      {eur(echeanceTotal)}
                    </span>
                  </div>
                </div>
